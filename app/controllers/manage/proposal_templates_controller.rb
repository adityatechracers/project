module Manage
  class ProposalTemplatesController < BaseController
    ...
    def index
      @proposal_templates = @proposal_templates.not_deleted
    end

    def edit
      @sections = @proposal_template.section_templates.includes(:item_templates, :versions)
    end
    ...
    def update
      if @proposal_template.is_upload_your_own? && params[:proposal_template][:name] && @proposal_template.name != params[:proposal_template][:name]
        redirect_to edit_manage_proposal_template_path(@proposal_template),
          :notice => "This proposal template name is not editable"
          return
      end
      ...
      if @proposal_template.update_attributes params[:proposal_template]
        ...
        render params[:proposal_template].has_key?(:agreement) ? :edit_contract : :edit
      end
    end

    def create
      if @proposal_template.save params[:proposal_template]
        redirect_to edit_manage_proposal_template_path(@proposal_template),
          :notice => "The proposal template has been created"
      else
        render :new
      end
    end
    ...
    def destroy
      ...
      if Proposal.where(:proposal_template_id => params[:id]).any?
        redirect_to manage_proposal_templates_path(params[:proposal_template_id]),
    ...
    def restore
      proposal_template = @proposal_template.versions.find(params[:version_id]).reify
      # The original version can not really be 'reified' (returns nil). To get
      # the original, you'd need to reify the second version.
    ...
    def reorder_sections
      params[:section_positions].each do |section_id, section_position|
        ...
        if params[:item_positions].present? && params[:item_positions].has_key?(section_id)
    ...
    def load_versions
      ...
      @sections = @proposal_template.section_templates.includes(:versions)
      respond_to do |format|
        format.js
      end
    end
